...
namespace MyNet.DynamicData.Extensions
{
    public static class DynamicDataExtensions
    {
        ...
        public static IObservable<IChangeSet<TDestination>> MergeManyEx<T, TDestination>(
               ...
               Func<T, IObservable<IChangeSet<TDestination>>> observableSelector) where T : notnull where TDestination : notnull => source == null
        ...
        /// <param name="source"></param>
        /// <param name="observableSelector"></param>
        /// <param name="observableKeySelector"></param>
        /// <returns></returns>
        public static IObservable<IChangeSet<TDestination, TDestinationKey>> MergeManyEx<T, TKey, TDestination, TDestinationKey>(
               ...
               Func<TDestination, TDestinationKey> observableKeySelector) where T : notnull where TDestination : notnull
            where TKey : notnull
        ...
        public static void Set<TObject, TKey>(this ISourceCache<TObject, TKey> source, IEnumerable<TObject> items) where TObject : notnull where TKey : notnull => source.Edit(x =>
        {
            x.Clear();
            x.AddOrUpdate(items);
        });
        ...
        public static IEnumerable<T> GetAddedItems<T>(this IChangeSet<T> changes) where T : notnull
        ...
        public static IEnumerable<T> GetRemovedItems<T>(this IChangeSet<T> changes) where T : notnull
        ...
        public static IDisposable SubscribeAll<T>(this IObservable<IChangeSet<T>> source, Action action) where T : notnull, INotifyPropertyChanged
        ...
        public static IObservable<TObject?> WhenExceptPropertyChanged<TObject>(this TObject source, params string[] propertiesToExclude)
            where TObject : INotifyPropertyChanged => source is null
                ? throw new ArgumentNullException(nameof(source))
                : Observable.FromEventPattern<PropertyChangedEventHandler?, PropertyChangedEventArgs>(handler => source.PropertyChanged += handler, handler => source.PropertyChanged -= handler).Where(x => !propertiesToExclude.Contains(x.EventArgs.PropertyName.OrEmpty())).Select(_ => source);
